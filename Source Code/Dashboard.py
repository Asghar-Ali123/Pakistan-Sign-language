...
classifier = load_model('asghar.h5')  # loading the model
...
def predictor():
    ...
    for i in range(len(fileEntry)):
        ...
        knn_matches = flann.knnMatch(desc_1, k=2)
        good_points = []
        ratio = 0.6
        ...
        if (abs(len(good_points) + len(
               knn_matches)) > 20):  # goodpoints and matcches sum from 1.png and customgestureimages is grater than 20
            gesname = fileEntry[i]
...
class Dashboard(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        movie = QtGui.QMovie("icons/psl2.gif")
